...
roj_stft_transform :: roj_stft_transform (roj_image_config a_conf, roj_complex_signal *a_window){
  ...
  /* allocate memory */
  m_spectrum = new complex double* [m_config.time.length];
  int byte_size = m_config.frequency.length * sizeof(complex double);
  for(int n=0; n<m_config.time.length; n++){

    m_spectrum[n] = fftw_alloc_complex(m_config.frequency.length);
...
roj_complex_signal* roj_stft_transform :: get_signal (){
  ...
  sig_config.rate = (double)(m_config.time.length - 1) / (m_config.time.max - m_config.time.min);
  sig_config.length = m_config.time.length;
  sig_config.start = m_config.time.min;
  ...
  for(int n=0; n<m_config.time.length; n++){
    ...
    print_progress(n+1, m_config.time.length, "rec");
  }
